...
import userexaption.DataTimeNotCorrect;
import userexaption.DataUserNotCorrect;
import userexaption.PhoneNumberNotCorrect;
import userexaption.SexNotCorrect;
...
public class DataUserInput implements Serializable {
    ...
    String data;
    public DataUserInput() throws Exception
    {
    ...
    String dataUserInputInfo() throws PhoneNumberNotCorrect, DataTimeNotCorrect, SexNotCorrect, DataUserNotCorrect {
        System.out.println("Введите строку в формате Фамилия Имя Отчество дата рождения (dd.mm.yyyy) пол(f/m)" +
                " номер телефона через пробел ");
        ...
        if (phoneNumber == 0) {
            ...
            throw new PhoneNumberNotCorrect(" Не верный номер телефона! Количество цифр в номере телефона > или <  7 ");
        }else if(birthDay == null||birthDay.isAfter(LocalDate.now())){
            throw new DataTimeNotCorrect("Не верный формат даты, либо дата больше текущей!");
        }else if(male == null){
            throw new SexNotCorrect("Не определен пол, либо не корректно введен f/m");
        }else if(info.size()!=3){
            throw new DataUserNotCorrect(" Не верно введены Ф.И.О.! Они должны содержать только английские буквы! " +
